...
def generate_goal():
    global in_start, cluster_map, robot_odometry, corners, end, client
    print("generating goal")
    next_goal = -1
    corners = [[7, 7], [7, int(cluster_map.shape[1]) - 7], [int(cluster_map.shape[0] - 7), 7],
               [int(cluster_map.shape[0]) - 7, int(cluster_map.shape[1]) - 7]]
    cluster_mid_x = cluster_map.shape[0] // 2
    cluster_mid_y = cluster_map.shape[1] // 2
    if cluster_map[cluster_mid_y][cluster_mid_x] != -1:
        in_start = True
    if not end:
        if not in_start:
            in_map = convert_from_cluster_to_map(cluster_mid_y, cluster_mid_x)
            goal_y, goal_x = convert_from_map_to_robot(in_map[0], in_map[1])
            in_start = True
            print("start : " + str(goal_y) + "          " + str(goal_x))
            move_base_client(goal_y, goal_x)
        else:
            for corner in corners:
                if cluster_map[corner[0], corner[1]] == -1:
                    next_goal = corners.index(corner)
                    break
            if next_goal == -1:
                unknowns_y, unknowns_x = np.where(cluster_map == -1)
                unknowns_y = np.append(unknowns_y[unknowns_y >= 3], unknowns_y[unknowns_y <= cluster_map.shape[0] - 3])
                unknowns_x = np.append(unknowns_x[unknowns_x >= 3], unknowns_x[unknowns_x <= cluster_map.shape[1] - 3])
                if len(np.where(cluster_map == -1)[0]) == 0:
                    print("CLuster cleared :)")
                    client.cancel_all_goals()
                    end = True
                    return
                random_unknown = random.Random().randint(0, len(unknowns_x) - 1)
                map_y, map_x = convert_from_cluster_to_map(unknowns_y[random_unknown], unknowns_x[random_unknown])
                goal_y, goal_x = convert_from_map_to_robot(map_y, map_x)
                move_base_client(goal_y, goal_x)
                print("random : " + str(goal_y) + "          " + str(goal_x))
            else:
                corner = corners[next_goal]
                corners.remove(corner)
                map_y, map_x = convert_from_cluster_to_map(corner[0], corner[1])
                goal_y, goal_x = convert_from_map_to_robot(map_y, map_x)
                move_base_client(goal_y, goal_x)
                print("corners : " + str(goal_y) + "          " + str(goal_x))
